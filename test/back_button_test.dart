...
void main() {
  ...
  for (final screen in <AppScreen>[AppScreen.stats, AppScreen.friends, AppScreen.settings]) {
    testWidgets('back from ${screen.name} returns to the lobby', (tester) async {
      ...
      expect(
        ...
        AppScreen.lobby,
        reason:
            'Back on ${screen.name} must return to the lobby, not quit the '
  ...
  testWidgets('back from the table leaves the table', (tester) async {
    ...
    expect(
      ...
      isNull,
      reason:
          'leaving must go through exitTable, which also cancels the '
          "round's timers — not a bare screen change",
    );
  });

  testWidgets('back peels the chat sheet before leaving the table', (tester) async {
    final notifier = await pumpAppOn(tester, AppScreen.table);
    ...
    expect(notifier.state.screen, AppScreen.table, reason: 'one press closes one layer; the player stays at the table');
  });
}
